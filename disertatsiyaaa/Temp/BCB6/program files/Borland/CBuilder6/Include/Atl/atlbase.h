...
#define _ATL_NO_FORCE_LIBS        // Eliminate #pragma comment(lib, xxxx)
#ifdef USING_ATLVCL
#define _ATL_NO_DEBUG_CRT         // avoid including (CRTDBG.H). Needed functions are defined in UTILCLS.H
#endif // USING_ATLVCL
#endif // __BORLANDC__
...
#else
extern HRESULT AtlSetChildSite(IUnknown* punkChild, IUnknown* punkParent);
#endif
...
#if !defined(__BORLANDC__) // MS_BUG
...
#if defined(__BORLANDC__) && defined(_GUIDDEF_H_)
...
{\
        __emit__(0x8b, 0x44, 0x24, 0x04);\
        if (*(unsigned long*)(_EAX + 8) <= 0) {\
          atlBadThunkCall();\
        }\
        \
        __emit__(0x8b, 0x44, 0x24, 0x04);\
        _EAX = *(unsigned long*)(_EAX + 4);\
        __emit__(0x89, 0x44, 0x24, 0x04);\
        _EAX = *(unsigned long*)(_EAX);\
        _EAX = *(unsigned long*)(_EAX + 4 * n);\
        __emit__(0xff, 0xe0);\
        return 0; /* can never be executed. suppresses warning. */ \
}
#else
#define IMPL_THUNK(n)\
__declspec(naked) inline HRESULT _QIThunk::f##n()\
...
#else
	// Initialized data kills precompiled headers, but string literals
	// are fine.  macros are undef'd below.
	#define szProgID _T("ProgID")
	#define szVIProgID _T("VersionIndependentProgID")
	#define szLS32 _T("LocalServer32")
	#define szIPS32 _T("InprocServer32")
	#define szThreadingModel _T("ThreadingModel")
	#define szAUTPRX32 _T("AUTPRX32.DLL")
	#define szApartment _T("Apartment")
	#define szBoth _T("both")
#endif // ! __BORLANDC__
	USES_CONVERSION;
	HRESULT hRes = S_OK;
	TCHAR szDesc[256];
...
#else
extern UINT WINAPI AtlGetDirLen(LPCOLESTR lpszPathName);
#endif
