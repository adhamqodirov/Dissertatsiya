...
#ifndef __NO_ALGORITHM_REPLACE
 /* The ANSI/ISO C++ Specification indicates that the function min()
    should be defined in the standard template library header algorithm.h.
    Previously, it was defined in the header stdlib.h. Because atlcom
    uses this function, and to avoid pulling in the new STL, that
    function is redefined here with a different name. If you wish to
    use the definition in algorithm.h, then define _NO_ALGORITHM_REPLACE
    prior to this block. Alternately, if you are for some reason 
    not compiling this with bcc32, you *may* need to include
    algorithm.h in order for the file to compile. */

#undef min
template <class _T> inline const _T _FAR &min_( const _T _FAR &__t1, const _T _FAR &__t2 )
{
    if  (__t1 < __t2)
        return __t1;
    else
...
inline unsigned long min_ (unsigned long __a, unsigned long __b)
{
    return min_<unsigned long> (__a, __b);
}
#endif
#endif
...
/* C++Builder does not automagically use __uuidof when
...
#ifdef __BORLANDC__
#define COM_IMPL_INTERFACE_ENTRY(x, y)\
        {&_ATL_IIDOF(x), \
        offsetofclass(x, y), \
        _ATL_SIMPLEMAPENTRY},
#endif
...
#ifdef __BORLANDC__
#define BEGIN_HEADER_COM_MAP(x) public:\
            typedef x _ComMapClass; \
            static HRESULT WINAPI _Cache(void* pv, REFIID iid, void **ppvObject, DWORD dw); \
            IUnknown* _GetRawUnknown(); \
            _ATL_DECLARE_GET_UNKNOWN(x)\
...
#define BEGIN_IMPL_COM_MAP(x, y) \
        HRESULT WINAPI y::_Cache(void* pv, REFIID iid, void** ppvObject, DWORD dw)\
	{\
		x* p = (x*)pv;\
		p->Lock();\
		HRESULT hRes = CComObjectRootBase::_Cache(pv, iid, ppvObject, dw);\
	...
	IUnknown* y::_GetRawUnknown()\
	{ ATLASSERT(_GetEntries()[0].pFunc == _ATL_SIMPLEMAPENTRY); return (IUnknown*)((int)this+_GetEntries()->dw); } \
        HRESULT y::_InternalQueryInterface(REFIID iid, void** ppvObject) \
        {\
                return InternalQueryInterface(this, _GetEntries(), iid, ppvObject); \
        }\
	const _ATL_INTMAP_ENTRY* WINAPI y::_GetEntries() { \
	static const _ATL_INTMAP_ENTRY _entries[] = { DEBUG_QI_ENTRY(x)
 
#define END_HEADER_COM_MAP() \
	virtual ULONG STDMETHODCALLTYPE AddRef( void) = 0; \
	virtual ULONG STDMETHODCALLTYPE Release( void) = 0; \
	STDMETHOD(QueryInterface)(REFIID, void**) = 0;

#ifdef _ATL_DEBUG
#define END_IMPL_COM_MAP() {NULL, 0, 0}}; return &_entries[1];}
#else
#define END_IMPL_COM_MAP() {NULL, 0, 0}}; return _entries;}
#endif //_ATL_DEBUG
#endif //BORLANDC
...
public:
	...
	BEGIN_COM_MAP(CComClassFactory)
        ...
        END_COM_MAP()
#else
        BEGIN_HEADER_COM_MAP(CComClassFactory)
        END_HEADER_COM_MAP()
#endif
...
public:
	...
	typedef CComClassFactory2<license> _ComMapClass;
       // _ComMapClass will be redeclared by the macro BEGIN_COM_MAP.
       // BCC32 doesn't like it when you do that. RAID-68269
#endif
...
public:
	...
	END_COM_MAP()
        #else
        BEGIN_HEADER_COM_MAP(CComClassFactoryAutoThread)
        END_HEADER_COM_MAP()
        #endif
				...
				if (SUCCEEDED(spTypeInfo->QueryInterface(&spTypeInfo2)))
					spInfo = spTypeInfo2;
                                ...
                                if (SUCCEEDED(spTypeInfo->QueryInterface(IID_ITypeInfo2, (void**)&spTypeInfo2)))
                                        spInfo = spTypeInfo2;
                                #endif
                                /* for some reason BCC32 doesn't like the first line and is not finding the correct inline overload of CComPtr::QI. */
		...
		*ppEnum = NULL;
		...
		if (SUCCEEDED(hRes))
		{
			...
			hRes = p->Init(m_begin, m_end, (m_dwFlags & BitCopy) ? this : m_spUnk);
                        #else
                        hRes = p->Init(m_begin, m_end, (m_dwFlags & BitCopy) ? (IUnknown*)this : (IUnknown*)m_spUnk);
                        #endif
			if (SUCCEEDED(hRes))
			{
				p->m_iter = m_iter;
...
CComAutoThreadModule<ThreadAllocator>::~CComAutoThreadModule()
{
	for (int i=0; i < m_nThreads; i++)
	{
    while ( ::PostThreadMessage(m_pApartments[i].m_dwThreadID, WM_QUIT, 0, 0) ) ::Sleep(100);
